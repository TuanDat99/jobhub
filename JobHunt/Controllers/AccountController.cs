...
namespace JobHunt.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(string type, LoginViewModel model, string returnUrl)
        {
            ...
            {
                return Json(new { message = "Wrong format. Please re-enter!", status = "warn" }, JsonRequestBehavior.AllowGet);
            }
            if (string.IsNullOrEmpty(type))
                return Json(new { message = "Please select the user you want to login!", status = "warn" }, JsonRequestBehavior.AllowGet);
            // This doesn't count login failures towards account lockout
            // To enable password failures to trigger account lockout, change to shouldLockout: true
            var userFind = UserManager.FindByEmail(model.Email);
            if (userFind == null)
                return Json(new { message = "Email does not exist!", status = "error" }, JsonRequestBehavior.AllowGet);
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    {
                        ...
                        if (UserManager.IsInRole(userFind.Id, nameRole))
                        {
                            ...
                            {
                                messageResult = "Check Your Email and User!";
                                statusResult = "error";
                                AuthenticationManager.SignOut(DefaultAuthenticationTypes.ApplicationCookie);
                            }
                            else
                            {
                                messageResult = "Login Succesfull!";
                                statusResult = "success";
                            }
                        }
                        else
                        {
                            messageResult = "Account or password is incorrect";
                            statusResult = "error";
                            AuthenticationManager.SignOut(DefaultAuthenticationTypes.ApplicationCookie);
                        }
                ...
                default:
                    return Json(new { message = "Wrong username or password!", status = "error" }, JsonRequestBehavior.AllowGet);
            }
        }
        ...
        public async Task<ActionResult> Register(string type, RegisterViewModel model)
        {
            ...
            {
                if (string.IsNullOrEmpty(type))
                    return Json(new { message = "Please select the user you want to register!", status = "warn" }, JsonRequestBehavior.AllowGet);
                ...
                if (result.Succeeded)
                {
                    ...
                    return Json(new { message = "Sign Up Success!, Login now", status = "success" }, JsonRequestBehavior.AllowGet);
                }
                else
                {
